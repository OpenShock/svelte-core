<script lang="ts">
  ...
  interface Props {
    ...
    Icon?: AnyComponent;
    /** When set, shows a "Forgot your password?" link pointing here. */
    forgotHref?: string;
    /** Called if the have-i-been-pwned lookup fails (the check is non-blocking). */
    onPwnedCheckError?: (error: unknown) => void;
  }
...
>
  {#snippet labelSnippet(id: string)}
    <div class="flex items-center">
      ...
      {#if forgotHref}
        <a href={forgotHref} class="ms-auto text-sm underline-offset-4 hover:underline">
