  ...
  {#snippet children({ thumbItems })}
    ...
    {#each thumbItems as thumb (thumb.index)}
      <SliderPrimitive.Thumb
        ...
        index={thumb.index}
        class="relative block size-3 shrink-0 cursor-w-resize rounded-full border border-ring bg-white ring-ring/50 transition-[color,box-shadow] select-none after:absolute after:-inset-2 hover:ring-3 focus-visible:ring-3 focus-visible:outline-hidden active:ring-3 disabled:pointer-events-none disabled:opacity-50"
      />
    {/each}
  {/snippet}
</SliderPrimitive.Root>
